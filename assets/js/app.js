...
(function initBuilderPage() {
  ...
  function updatePreview() {
    ...
    setElementText('previewTime', formatTime(cleanValue('time')));
    ...
    PREBUILT_PLANS = [
      {
        ...
        label: 'Coffee & a little adventure',
        savedAt: new Date('2026-01-15').toISOString(),
        fields: { email: '', type: 'date', title: 'Coffee and a little adventure', date: offset(7), time: '15:00', place: 'Cafe Luna, downtown', todo: 'Coffee first, then a walk somewhere pretty.', message: 'I picked this because it made me think of you.', theme: 'blush' },
      },
      {
        id: 'prebuilt-brunch',
        label: 'Weekend Brunch',
        savedAt: new Date('2026-02-01').toISOString(),
        fields: { email: '', type: 'hangout', title: 'Weekend Brunch', date: offset(3), time: '10:30', place: 'Sunny Side Bistro', todo: 'Brunch, then maybe browse the farmer\'s market.', message: 'It\'s been too long! Let\'s catch up.', theme: 'sunrise' },
      },
      {
        id: 'prebuilt-picnic',
        label: 'Park Picnic',
        savedAt: new Date('2026-03-10').toISOString(),
        fields: { email: '', type: 'hangout', title: 'Park Picnic', date: offset(5), time: '12:00', place: 'Riverside Park', todo: 'Pack a basket, bring a blanket, enjoy the sun.', message: 'The weather\'s supposed to be beautiful!', theme: 'garden' },
      },
      {
        id: 'prebuilt-museum',
        label: 'Museum Afternoon',
        savedAt: new Date('2026-04-05').toISOString(),
        fields: { email: '', type: 'date', title: 'Museum Afternoon', date: offset(10), time: '14:00', place: 'City Art Museum', todo: 'Explore the new exhibit, then grab dinner nearby.', message: 'Thought you\'d love the new Impressionist exhibit.', theme: 'midnight' },
      },
      {
        id: 'prebuilt-movie',
        label: 'Movie & Dinner',
        savedAt: new Date('2026-05-01').toISOString(),
        fields: { email: '', type: 'date', title: 'Movie & Dinner', date: offset(2), time: '18:30', place: 'Cinema 9 & Olive Garden', todo: 'Catch the 7pm show, then dinner after.', message: 'Heard great things about this film — want to join?', theme: 'blush' },
      },
      {
        id: 'prebuilt-flirt-date',
        label: 'Sunset Drinks',
        savedAt: new Date('2026-06-01').toISOString(),
        fields: { email: '', type: 'date', title: 'Sunset Drinks', date: offset(4), time: '17:30', place: 'Skyline Rooftop Bar', todo: 'Watch the sunset, sip cocktails, see where the night goes.', message: 'I know a place with a view that deserves someone like you.', theme: 'flirt' },
      },
    ];
  })();
  ...
  if (loadPlanButton) {
    loadPlanButton.addEventListener('click', function() {
      ...
      if (fields.time) fields.time.value = f.time || '';
...
(function initInvitationPage() {
  ...
  function renderInvite(invite) {
    ...
    setElementText('inviteTime', formatTime(invite.time));
  ...
  responseForm.addEventListener('submit', async function(event) {
    ...
    var answer = event.submitter ? event.submitter.value : 'yes';
...
(function initThankPage() {
  ...
  function notifyInviter() {
    ...
    var time = (inviteData && inviteData.time) || '';
